...
CREATE PROCEDURE [30MDS_ods].[usp_Insertmdm__Countries]
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS (SELECT count(*) FROM [30MDS_ods].[mdm__Countries] HAVING count(*) > 0)
	BEGIN
	    UPDATE [30MDS_stg].[mdm__Countries]
	...
	*/
	...
	IF EXISTS (SELECT count(*) FROM [30MDS_ods].[mdm__Countries] HAVING count(*) > 0)
	BEGIN

		UPDATE A
		   SET A.[ODSRecordStatus] = 'N'
		  FROM [30MDS_stg].[mdm__Countries] A LEFT JOIN [30MDS_ods].[mdm__Countries] B
		    ON A.[ID] = B.[ID]
		 WHERE B.[ID] IS NULL

		UPDATE A
		   SET A.[ODSRecordStatus] = 'C'
		  FROM [30MDS_stg].[mdm__Countries] A INNER JOIN [30MDS_ods].[mdm__Countries] B
		    ON A.[ID] = B.[ID]
	...
	*/
	...
	UPDATE A
	  ...
	  FROM [30MDS_ods].[mdm__Countries] A INNER JOIN [30MDS_stg].[mdm__Countries] B
	    ON A.[ID] = B.[ID]
	...
	*/
	...
	SET @CommitCount = (SELECT COUNT(*) FROM [30MDS_stg].[mdm__Countries])
