...
CREATE PROCEDURE [21D365_ods].[usp_Insertecorescategoryhierarchy]
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS (SELECT count(*) FROM [21D365_ods].[ecorescategoryhierarchy] HAVING count(*) > 0)
	BEGIN
	    UPDATE [21D365_stg].[ecorescategoryhierarchy]
	...
	*/
	...
	IF EXISTS (SELECT count(*) FROM [21D365_ods].[ecorescategoryhierarchy] HAVING count(*) > 0)
	BEGIN

		UPDATE A
		   SET A.[ODSRecordStatus] = 'N'
		  FROM [21D365_stg].[ecorescategoryhierarchy] A LEFT JOIN [21D365_ods].[ecorescategoryhierarchy] B
		    ON A.[recid] = B.[recid]
		 WHERE B.[recid] IS NULL

		UPDATE A
		   SET A.[ODSRecordStatus] = 'C'
		  FROM [21D365_stg].[ecorescategoryhierarchy] A INNER JOIN [21D365_ods].[ecorescategoryhierarchy] B
		    ON A.[recid] = B.[recid]
	...
	*/
	...
	UPDATE A
	  ...
	  FROM [21D365_ods].[ecorescategoryhierarchy] A INNER JOIN [21D365_stg].[ecorescategoryhierarchy] B
	    ON A.[recid] = B.[recid]
	...
	*/
	...
	INSERT INTO [21D365_ods].[ecorescategoryhierarchy]			(			 [Id],			 [SinkCreatedOn],			 [SinkModifiedOn],			 [hierarchymodifier],			 [sysdatastatecode],			 [name],			 [modifieddatetime],			 [modifiedby],			 [modifiedtransactionid],			 [createddatetime],			 [createdby],			 [createdtransactionid],			 [dataareaid],			 [recversion],			 [partition],			 [sysrowversion],			 [recid],			 [tableid],			 [versionnumber],			 [createdon],			 [modifiedon],			 [IsDelete],			 [PartitionId],			 [Ingestion_Timestamp],			 [ODSStartDate],			 [ODSEndDate],			 [ODSActive],			 [ODSHashValue]			) 	  SELECT [Id],			 [SinkCreatedOn],			 [SinkModifiedOn],			 [hierarchymodifier],			 [sysdatastatecode],			 [name],			 [modifieddatetime],			 [modifiedby],			 [modifiedtransactionid],			 [createddatetime],			 [createdby],			 [createdtransactionid],			 [dataareaid],			 [recversion],			 [partition],			 [sysrowversion],			 [recid],			 [tableid],			 [versionnumber],			 [createdon],			 [modifiedon],			 [IsDelete],			 [PartitionId],			 [Ingestion_Timestamp],			 GETUTCDATE(),			 CONVERT(DATETIME, '31-12-9999', 103),			 CONVERT(BIT, 1),			 [ODSHashValue] 		FROM [21D365_stg].[ecorescategoryhierarchy] WHERE [ODSRecordStatus] IN ('C', 'N');					   
