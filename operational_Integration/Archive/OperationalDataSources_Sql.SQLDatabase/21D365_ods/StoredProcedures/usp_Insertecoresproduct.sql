...
CREATE PROCEDURE [21D365_ods].[usp_Insertecoresproduct]
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS (SELECT count(*) FROM [21D365_ods].[ecoresproduct] HAVING count(*) > 0)
	BEGIN
	    UPDATE [21D365_stg].[ecoresproduct]
	...
	*/
	...
	IF EXISTS (SELECT count(*) FROM [21D365_ods].[ecoresproduct] HAVING count(*) > 0)
	BEGIN

		UPDATE A
		   SET A.[ODSRecordStatus] = 'N'
		  FROM [21D365_stg].[ecoresproduct] A LEFT JOIN [21D365_ods].[ecoresproduct] B
		    ON A.[recid] = B.[recid]
		 WHERE B.[recid] IS NULL

		UPDATE A
		   SET A.[ODSRecordStatus] = 'C'
		  FROM [21D365_stg].[ecoresproduct] A INNER JOIN [21D365_ods].[ecoresproduct] B
		    ON A.[recid] = B.[recid]
	...
	*/
	...
	UPDATE A
	  ...
	  FROM [21D365_ods].[ecoresproduct] A INNER JOIN [21D365_stg].[ecoresproduct] B
	    ON A.[recid] = B.[recid]
