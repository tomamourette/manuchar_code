...
CREATE PROCEDURE [21D365_ods].[usp_Insertgeneraljournalentry]
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS (SELECT count(*) FROM [21D365_ods].[generaljournalentry] HAVING count(*) > 0)
	BEGIN
	    UPDATE [21D365_stg].[generaljournalentry]
	...
	*/
	...
	IF EXISTS (SELECT count(*) FROM [21D365_ods].[generaljournalentry] HAVING count(*) > 0)
	BEGIN

		UPDATE A
		   SET A.[ODSRecordStatus] = 'N'
		  FROM [21D365_stg].[generaljournalentry] A LEFT JOIN [21D365_ods].[generaljournalentry] B
		    ON A.[recid] = B.[recid]
		 WHERE B.[recid] IS NULL

		UPDATE A
		   SET A.[ODSRecordStatus] = 'C'
		  FROM [21D365_stg].[generaljournalentry] A INNER JOIN [21D365_ods].[generaljournalentry] B
		    ON A.[recid] = B.[recid]
	...
	*/
	...
	UPDATE A
	  ...
	  FROM [21D365_ods].[generaljournalentry] A INNER JOIN [21D365_stg].[generaljournalentry] B
	    ON A.[recid] = B.[recid]
