...
CREATE PROCEDURE [dbo].[usp_InsertVendorTransactions]
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS (SELECT count(*) FROM [D365_FO].[VendorTransactions] HAVING count(*) > 0)
	BEGIN
	    UPDATE [Stg_D365_FO].[vendtrans]
	...
	*/
	...
	IF EXISTS (SELECT count(*) FROM [D365_FO].[VendorTransactions] HAVING count(*) > 0)
	BEGIN

		UPDATE A
		   SET A.[ODSRecordStatus] = 'N'
		  FROM [Stg_D365_FO].[vendtrans] A LEFT JOIN [D365_FO].[VendorTransactions] B
		    ON A.[recid] = B.[recid]
		 WHERE B.[recid] IS NULL

		UPDATE A
		   SET A.[ODSRecordStatus] = 'C'
		  FROM [Stg_D365_FO].[vendtrans] A INNER JOIN [D365_FO].[VendorTransactions] B
		    ON A.[recid] = B.[recid]
	...
	*/
	...
	UPDATE A
	  ...
	  FROM [D365_FO].[VendorTransactions] A INNER JOIN [Stg_D365_FO].[vendtrans] B
	    ON A.[recid] = B.[recid]
