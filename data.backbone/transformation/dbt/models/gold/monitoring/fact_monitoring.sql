...
monitoring_operations AS (
    SELECT
        ...
        semantic_model_name
    FROM {{ ref('brg_monitoring_operations') }}
),
...
SELECT
    child_dim.tkey_pipeline AS tkey_child_pipeline,
    parent_dim.tkey_pipeline AS tkey_parent_pipeline,

    fm.pipeline_run_id,
    fm.pipeline_status,

    CAST(fm.pipeline_start_time_utc AS datetime2(6)) AS pipeline_start_time_utc,
    CAST(fm.pipeline_end_time_utc AS datetime2(6)) AS pipeline_end_time_utc,
    CAST(fm.pipeline_scheduled_time_utc AS datetime2(6)) AS pipeline_scheduled_time_utc,

    fm.orchestration_run_id,
    fm.operation,
    fm.operation_status,
    
    CAST(fm.operation_start_time_utc AS datetime2(6)) AS operation_start_time_utc,
    CAST(fm.operation_end_time_utc AS datetime2(6)) AS operation_end_time_utc,
...
FROM fact_monitoring fm
LEFT JOIN {{ ref('dim_pipeline') }} AS child_dim
...
LEFT JOIN {{ ref('dim_pipeline') }} AS parent_dim
