WITH source_dynamics_ecoresproduct AS (
    ...
    FROM {{ ref('product_global_dynamics') }} AS pd
), 
...
source_mds_product AS (
    ...
    FROM {{ ref('product_global_mds') }} AS pm
), 
...
sources_deduplicated AS (
    SELECT 
        ...
        ldts,
        record_source,
        ROW_NUMBER() OVER (PARTITION BY bkey_product_global_source ORDER BY ldts ASC) AS row_number
