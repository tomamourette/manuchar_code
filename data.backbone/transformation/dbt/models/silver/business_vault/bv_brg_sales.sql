...
SELECT 
    ...
    END AS bkey_site,
    inv.sales_invoice_transactional_currency_code AS bkey_currency_transactional,
    ...
    NULL AS sales_accrual_transactional_currency_code,
    'USD' AS sales_accrual_group_currency_code,
    NULL AS sales_accrual_functional_currency_code,
...
FROM {{ ref('bv_sales_invoice_line')}} invl 
LEFT JOIN {{ ref('bv_sales_invoice')}} inv
    ON inv.bkey_sales_invoice = invl.bkey_sales_invoice
LEFT JOIN {{ ref('bv_sales_invoice_line_cogs')}} ivcg
...
SELECT 
    ...
    NULL AS bkey_site,
    NULL AS bkey_currency_transactional,
    NULL AS bkey_currency_group,
    NULL AS bkey_currency_functional,
    ...
    sales_order_currencycode AS sales_accrual_transactional_currency_code,
    'USD' AS sales_accrual_group_currency_code,
    NULL AS sales_accrual_functional_currency_code,
...
SELECT 
    ...
    NULL AS sales_accrual_volume_mt,
    NULL AS sales_accrual_transactional_currency_code,
    NULL AS sales_accrual_group_currency_code,
    NULL AS sales_accrual_functional_currency_code,
