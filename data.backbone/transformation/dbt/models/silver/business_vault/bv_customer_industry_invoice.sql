...
WITH source_customer_industry_invoice AS (
    SELECT
        ...
        sat.valid_from,
        sat.valid_to
    FROM {{ ref('rv_hub_customer_industry_invoice')}} hub
    LEFT JOIN {{ ref('rv_sat_customer_industry_invoice')}} sat ON hub.bkey_customer_industry_invoice_source = sat.bkey_customer_industry_invoice_source
