WITH sales_invoice_line_cogs AS (
    SELECT
        ...
        sat.valid_to,
        sat.is_current
    FROM {{ ref('rv_hub_sales_invoice_line_cogs')}} hub
    LEFT JOIN {{ ref('rv_sat_sales_invoice_line_cogs')}} sat ON hub.bkey_sales_invoice_line_cogs_source = sat.bkey_sales_invoice_line_cogs_source
)
