...
WITH mds_age_groups_history AS (
	-- Entity details over time (SCD2)
    SELECT
        Code + '_MDS' AS bkey_age_group_source,
        Code AS bkey_age_group,
        'MDS' AS bkey_source,
        Name,
        ID,
        AgeGroupMinDays,
...
timeranges AS (
    SELECT
        ...
        CAST(COALESCE(LEAD(valid_from) OVER (PARTITION BY bkey_age_group ORDER BY valid_from),'2999-12-31 23:59:59.999999') AS DATETIME2(6)) AS valid_to
    FROM (
        SELECT bkey_age_group_source, bkey_age_group, bkey_source, valid_from, valid_to FROM mds_age_groups_history
    ) AS time_events
)
...
SELECT 
    tr.bkey_age_group_source,
    tr.bkey_age_group,
    tr.bkey_source,

    -- From Main Entity History Table (Time-Dependent Attributes)
    e1.Name AS age_group_name,
    e1.ID AS age_group_id,
    e1.AgeGroupMinDays AS age_group_min_days,
    e1.AgeGroupMaxDays AS age_group_max_days,
    e1.AgeGroupPosition AS age_group_sort,
...
LEFT JOIN mds_age_groups_history e1 
ON e1.bkey_age_group_source = tr.bkey_age_group_source
