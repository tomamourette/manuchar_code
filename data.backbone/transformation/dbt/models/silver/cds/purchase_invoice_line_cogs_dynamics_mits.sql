...
WITH purchase_invoice_line_cogs_history AS (
	SELECT
		ROW_NUMBER() OVER (ORDER BY lft.purchid, purl.linenumber, vij.invoiceid) AS tkey_purchase_invoice_line_cogs,
		lft.purchid + '_' + ISNULL(CONVERT(VARCHAR, purl.linenumber), '') + '_' + ISNULL(vij.invoiceid, '') + '_DYNAMICS' AS bkey_purchase_invoice_line_cogs_source,
		lft.purchid + '_' + ISNULL(CONVERT(VARCHAR, purl.linenumber), '') + '_' + ISNULL(vij.invoiceid, '') AS bkey_purchase_invoice_line_cogs,
		...
		CAST(LEAD(davc.modifieddatetime) OVER (PARTITION BY CONCAT(lft.purchid, '_', CONVERT(VARCHAR, purl.linenumber), '_', ISNULL(vij.invoiceid, '')) ORDER BY davc.modifieddatetime) AS DATETIME2(6)) AS valid_to
	FROM {{ ref('sv_dynamics_lgslogisticfiletable')}} lft 
	LEFT JOIN {{ ref('sv_dynamics_lgslogisticfileline')}} lfl
		ON lft.logisticfileid = lfl.logisticfileid
	...
	LEFT JOIN {{ ref('sv_dynamics_dimensionattributevaluecombination')}} davc
		ON lfl.logisticfileid = davc.logisticsfilevalue
	...
	LEFT JOIN {{ ref('currency_rate_mona') }} curr 
		ON curr.currency_rate_currency_code = vij.currencycode
    	AND FORMAT(vij.invoicedate, 'yyyyMM') = FORMAT(curr.currency_rate_closing_date, 'yyyyMM')
	WHERE davc.mainaccountvalue <> 700000
	GROUP BY 
		lft.logisticfileid,
		pur.paymentsched,
		pur.payment,
