...
WITH mds_industry_history AS (
	-- Entity details over time (SCD2)
    SELECT
	    Code + '_MDS' AS bkey_industry_source,
        Code AS bkey_industry,
        'MDS' AS bkey_source,
        Name,
        [Group],
...
timeranges AS (
    SELECT 
        ...
        CAST(COALESCE(LEAD(valid_from) OVER (PARTITION BY bkey_industry ORDER BY valid_from),'2999-12-31 23:59:59.999999') AS DATETIME2(6)) AS valid_to
    FROM (
        SELECT bkey_industry_source, bkey_industry, bkey_source, valid_from, valid_to FROM mds_industry_history
    ) AS time_events
)
...
SELECT 
    tr.bkey_industry_source,
    tr.bkey_industry,
    tr.bkey_source,

    -- From Main Entity History Table (Time-Dependent Attributes)
    e1.Name AS industry_name,
    e1.[Group] AS industry_group,
...
LEFT JOIN mds_industry_history e1 
ON e1.bkey_industry_source = tr.bkey_industry_source
