...
WITH mds_uom_history AS (
	-- Entity details over time (SCD2)
    SELECT
        Code + '_MDS' AS bkey_unit_of_measure_source,
        Code AS bkey_unit_of_measure,
        'MDS' AS bkey_source,
        Name,
        Description,
        ConversionToMT,
...
timeranges AS (
    ...
    FROM (
        SELECT bkey_unit_of_measure_source, bkey_unit_of_measure, bkey_source, valid_from, valid_to FROM mds_uom_history
    ) AS time_events
)
...
SELECT 
    ...
    e1.Name AS unit_of_measure_name,
    e1.Description AS unit_of_measure_description,
    e1.ConversionToMT AS unit_of_measure_conversion_to_mt,
