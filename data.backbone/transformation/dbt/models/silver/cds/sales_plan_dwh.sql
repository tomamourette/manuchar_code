...
WITH PlanData AS (
    ...
    FROM {{ ref("sv_dwh_anaplan_budget") }} ab
    LEFT JOIN {{ ref("sv_dwh_dim_customer") }}  cus ON CAST(cus.KDP_SK AS VARCHAR) = ab.FK_Customer
    LEFT JOIN {{ ref("sv_dwh_dim_product") }}  p ON CAST(p.KDP_SK AS VARCHAR) = ab.FK_Product
...
BudgetOther AS (
    SELECT DISTINCT
        ...
        CONVERT(FLOAT, ab.PlanRate) AS [PlanRate],
        ab.ingestion_timestamp
    FROM {{ ref("sv_dwh_dbo_anaplan_budget") }} ab
    LEFT JOIN {{ ref("sv_dwh_dim_customer") }}  cus ON CAST(cus.KDP_SK AS VARCHAR) = ab.FK_Customer
    LEFT JOIN {{ ref("sv_dwh_dim_product") }}  p ON CAST(p.KDP_SK AS VARCHAR) = ab.FK_Product
...
Forecast AS (
    ...
    FROM {{ ref("sv_dwh_anaplan_forecast") }} af
    LEFT JOIN {{ ref("sv_dwh_dim_customer") }}  cus ON CAST(cus.KDP_SK AS VARCHAR) = af.FK_Customer
    LEFT JOIN {{ ref("sv_dwh_dim_product") }}  p ON CAST(p.KDP_SK AS VARCHAR) = af.FK_Product
    WHERE ISNUMERIC(MonthCode) = 1
    AND LEN(MonthCode) = 6
    AND RIGHT(MonthCode, 2) BETWEEN '01' AND '12'
),
...
FROM  {{ ref("sv_dwh_stg_sales_plan_hist") }} sh   
LEFT JOIN {{ ref("sv_mds_countries") }} cou
...
RankedSalesPlan AS (
    ...
    LEFT JOIN (
        SELECT 
            ...
            ctm.Customer_ID
        FROM {{ ref("sv_mds_customermapping") }} cm 
    ...
    LEFT JOIN {{ ref("sv_mds_product") }} p 
