...
WITH mds_currency_history AS (
	-- Entity details over time (SCD2)
    SELECT
	    Code + '_MDS' AS bkey_currency_source,
        Code AS bkey_currency,
        'MDS' AS bkey_source,
        Name,
...
timeranges AS (
    ...
    FROM (
        SELECT bkey_currency_source, bkey_currency, bkey_source, valid_from, valid_to FROM mds_currency_history
    ) AS time_events
)
...
LEFT JOIN mds_currency_history e1 
ON e1.bkey_currency = tr.bkey_currency 
