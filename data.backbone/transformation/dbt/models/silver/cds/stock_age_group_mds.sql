...
WITH mds_stock_age_groups_history AS (
	-- Entity details over time (SCD2)
    SELECT
        Code + '_MDS' AS bkey_stock_age_group_source,
        Code AS bkey_stock_age_group,
        'MDS' AS bkey_source,
        Name,
        Code,
        Age_Group_Min_Days,
    ...
    FROM {{ ref("sv_mds_stockagegroups") }}
),
...
timeranges AS (
    SELECT
        ...
        CAST(COALESCE(LEAD(valid_from) OVER (PARTITION BY bkey_stock_age_group ORDER BY valid_from),'2999-12-31 23:59:59.999999') AS DATETIME2(6)) AS valid_to
    FROM (
        SELECT bkey_stock_age_group_source, bkey_stock_age_group, bkey_source, valid_from, valid_to FROM mds_stock_age_groups_history
    ) AS time_events
)
...
SELECT 
    tr.bkey_stock_age_group_source,
    tr.bkey_stock_age_group,
    tr.bkey_source,

    -- From Main Entity History Table (Time-Dependent Attributes)
    e1.Name AS stock_age_group_name,
    e1.Code AS stock_age_group_id,
    e1.Age_Group_Min_Days AS stock_age_group_min_days,
    e1.Age_Group_Max_Days AS stock_age_group_max_days,
    e1.Age_Group_Position AS stock_age_group_sort,
...
LEFT JOIN mds_stock_age_groups_history e1 
ON e1.bkey_stock_age_group_source = tr.bkey_stock_age_group_source
