...
WITH mona_currency_history AS (
	-- Entity details over time (SCD2)
    SELECT
	    CurrCode + '_MONA' AS bkey_currency_source,
        CurrCode AS bkey_currency,
        'MONA' AS bkey_source,
        CurrName,
        CAST(ODSIngestionDate  AS DATETIME2(6)) AS valid_from,  -- Ensure precision
        CAST(LEAD(ODSIngestionDate) OVER (PARTITION BY CurrCode ORDER BY ODSIngestionDate) AS DATETIME2(6)) AS valid_to
    FROM {{ ref("sv_mona_ts017r2") }}
),
...
timeranges AS (
    SELECT 
        ...
        CAST(COALESCE(LEAD(valid_from) OVER (PARTITION BY bkey_currency ORDER BY valid_from),'2999-12-31 23:59:59.999999') AS DATETIME2(6)) AS valid_to
    FROM (
        SELECT bkey_currency_source, bkey_currency, bkey_source, valid_from, valid_to FROM mona_currency_history
    ) AS time_events
)
...
LEFT JOIN mona_currency_history e1 
ON e1.bkey_currency_source = tr.bkey_currency_source
