-- Auto Generated (Do not modify) 6413D43174DD5E0E01A05B478C1C75E8849B8734E4DCE025B3288D3EC22E6E30
create view "dds_finance"."fact_financial_statements_temp_view" as 

-- Mapping table for Volume Accounts
WITH volume_accounts AS (
	SELECT 
        bkey_account
    FROM "dbb_warehouse"."mim"."bv_account"
...
public_holidays AS (
    ...
    FROM "dbb_warehouse"."mim"."bv_public_holiday"
    GROUP BY 
		country_region_code, 
		MONTH(date),
		YEAR(date)
...
company_participation_percentage AS (
    ...
    FROM "dbb_warehouse"."dds_finance"."dim_date" dat
    INNER JOIN "dbb_warehouse"."dds_finance"."dim_company" com
...
fte_first_reporting_months AS (
	...
	FROM "dbb_warehouse"."mim"."bv_consolidated_amount" csa
	LEFT JOIN "dbb_warehouse"."mim"."bv_consolidation_period" csp
...
ytd_reporting_groups AS (
    ...
    FROM "dbb_warehouse"."mim"."bv_consolidated_amount" csa
    LEFT JOIN "dbb_warehouse"."dds_finance"."dim_consolidation_period" csp
        ON csa.consolidated_consolidation_period = csp.bkey_consolidation_period
    LEFT JOIN "dbb_warehouse"."mim"."bv_journal_mapping" jm 
...
SELECT 
    ...
    CASE 
        WHEN csp.consolidation_L4L_scope LIKE 'Like4Like%' 
            ...
            AND (
                csp.consolidation_year > 2025
                OR (csp.consolidation_year = 2025 AND csp.consolidation_month >= 2)
            )
        THEN 
            0
        ELSE 
            CAST(ytd.ytd_bundle_amount AS DECIMAL(18, 2))
    END AS bundle_amount,
    ...
    CASE 
        ...
        THEN
            CAST(ytd.ytd_bundle_amount AS DECIMAL(18, 2)) 
        ELSE 
            CAST(ytd.ytd_conso_adjusted_amount AS DECIMAL(18, 2))
    END AS conso_adjusted_amount,
...
LEFT JOIN "dbb_warehouse"."dds_finance"."dim_company" pcom
...
LEFT JOIN "dbb_warehouse"."dds_finance"."dim_currency" cur
    ON ytd.bkey_currency = cur.bkey_currency
LEFT JOIN "dbb_warehouse"."dds_finance"."dim_industry" ind
