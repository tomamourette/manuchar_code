-- Auto Generated (Do not modify) FA5D40D008546B46FF1FC271E23383C7822118942EE225CE01837F65538120F2
create view "dds_finance"."dim_account_bc_scd1" as 
...
dim_start_end AS (
    SELECT 
        ...
        --valid_to,
        --is_current
        min(src_load_dt) over (partition by bkey_account order by bkey_account, src_load_dt) as valid_from,
        '9999-12-31 00:00:00' as valid_to,
        1 as is_current,
        is_deleted,
        row_number() over(partition by bkey_account ORDER BY bkey_account, src_load_dt desc) rownum
    FROM dim_pre
)
select * 
    ,CONCAT('7175f2c7-e1aa-404a-b8b2-0ed494ecb03a','_','"dbb_warehouse"."dds_finance"."dim_account_bc_scd1"') as model_exec_id
from dim_start_end
where rownum = 1;