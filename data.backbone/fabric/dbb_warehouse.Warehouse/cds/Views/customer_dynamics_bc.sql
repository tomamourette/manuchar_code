-- Auto Generated (Do not modify) BF3FB19FB4AD1EEBA05AA86D684E8C465E5E6896DC503E9D509FA8BFDEF71C27
create view "cds"."customer_dynamics_bc" as -- ===============================
...
WITH dynamics_customer_custtable_history AS (
    SELECT 
        ...
        dct.modifiedon as modifiedon
    FROM "dbb_warehouse"."ods"."sv_dynamics_custtable_bc" dct
),
...
dynamics_customer_custgroup_current AS (
    ...
    FROM (
        ...
        FROM "dbb_warehouse"."ods"."sv_dynamics_custtable_bc" dct
        LEFT JOIN "dbb_warehouse"."ods"."sv_dynamics_custgroup" dcg ON dct.custgroup = dcg.custgroup
    ) AS sub
    WHERE rn = 1
),
...
dynamics_customer_party_current AS (
    ...
    FROM (
        ...
        FROM "dbb_warehouse"."ods"."sv_dynamics_custtable_bc" dct
        LEFT OUTER JOIN "dbb_warehouse"."ods"."sv_dynamics_dirpartytable" ddpt ON ddpt.recid = dct.party
    ) latest_values
    WHERE rn = 1
),
...
dynamics_customer_postal_address_current AS (
    ...
    FROM (
        ...
        FROM "dbb_warehouse"."ods"."sv_dynamics_custtable_bc" dct
        LEFT OUTER JOIN "dbb_warehouse"."ods"."sv_dynamics_dirpartytable" ddpt ON ddpt.recid = dct.party
        LEFT OUTER JOIN "dbb_warehouse"."ods"."sv_dynamics_dirpartylocation" ddpl ON ddpl.party = ddpt.recid AND ddpl.isprimary = 1
        LEFT OUTER JOIN "dbb_warehouse"."ods"."sv_dynamics_logisticslocation" dll ON dll.recid = ddpl.location
        LEFT OUTER JOIN "dbb_warehouse"."ods"."sv_dynamics_logisticspostaladdress" dlpa ON dlpa.location = dll.recid AND GETDATE() BETWEEN dlpa.validfrom AND dlpa.validto
    ) latest_values
    WHERE rn = 1
)
